class ToneCurveGui {
  ...
  drawAnsx() {
    let defaultLut = LutUtil.createDefaultLut()
    for (let i = 0; i < 4; i++) {
      
      if (i == 0) {
        for( let j = 1; j < 4; j++) {
          let ps = this.ansPoints[j]
          let isLine = ps.length == 2 && 
              ps[0].x == 0 &&
              ps[0].y == 0 &&
              ps[1].x == 255 && 
              ps[1].y == 255
  ...
  drawTab() {
    ...
    for(let i = 0; i < 4; i++) {
      ...
      rect(x, this.pos.y, this.buttonWidth, this.buttonHeight, corners[0], corners[1], corners[2], corners[3])
      fill("#FFFFFF")
